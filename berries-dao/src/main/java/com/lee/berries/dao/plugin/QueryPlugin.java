...
public class QueryPlugin implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		if(StatumentConstants.STATEMENT_GETONE_ID.endsWith(statement.getId())){ //查询单个，如果结果出现多个将报错！
		...
		if(StatumentConstants.STATEMENT_LIST_ID.endsWith(statement.getId())){ //查询多个记录！
			SqlSource sqlSource = new SelectSqlSource(statement.getConfiguration(), statement.getBoundSql(paramObject), paramObject);
			BerriesUtils.setFieldValue(statement, "sqlSource", sqlSource);
		}
		if(StatumentConstants.STATEMENT_ROW_ID.endsWith(statement.getId())){ //查询记录数！
